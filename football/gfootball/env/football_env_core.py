  ...
  def _convert_players_observation(self, players, name, result):
    ...
    for id, player in enumerate(players):
      if player.designated_player:
        designated_player = id
    for id, player in enumerate(players):
      # if we don't have a designated player
      if designated_player == -1 or id == designated_player:
        dist = 0
      else:
        dist = self._distance(np.array([player.position[0],player.position[1],player.position[2]]), np.array([players[designated_player].position[0],players[designated_player].position[1],players[designated_player].position[2]]))
      if dist <= self._config['env_radius']:
        positions.append(player.position[0])
        positions.append(player.position[1])
        directions.append(player.direction[0])
        directions.append(player.direction[1])
