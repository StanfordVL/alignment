...
class SACDMultiCommWMPolicy(BasePolicy):
    ...
    def __init__(self,
                 ...
                 comm_dist_fn: torch.distributions.Distribution
                 = torch.distributions.Categorical,
                 num_adv: int = 0,
                 intr_rew_options: dict = {"adv": False, "agt": True},
                 comm_agts: List[int] = [],
                 act_dims: List[int] = [],
                 **kwargs) -> None:
        ...
        self.comm_dist_fn = comm_dist_fn
        self.comm_agts = comm_agts
    ...
    def forward(self, batch: Batch,
        ...
        for i, actor in enumerate(self.actors):
            logit, h = actor(obs[:, i])
            dist = self.dist_fn(logit[:,:self.act_dims[i]])
            act = dist.sample()
            if i in self.comm_agts:
                comm_dist = self.comm_dist_fn(logit[:,self.act_dims[i]:])
                comm = comm_dist.sample()
            else:
                comm = torch.zeros(act.size(), dtype=torch.long, device=act.device)
            act = torch.cat([act.unsqueeze(1), comm.unsqueeze(1)],dim=-1)
    ...
    def calculate_intrinsic_reward(self, batch: Batch) -> Batch:
        ...
        if self.partial_obs:
            ...
            for i in range(self.num_adv, num_agents): # apply intr rew only on good agents
                obs_j_list = self.imagine_agent_j_obs(batch.obs[:,i], i, j_list)
                ...
                if self.intr_rew_options == '111': # intr rew from both both
                    ...
                    intr_rew[:, i] += (pred_losses[:, :self.num_adv].sum(dim=1) - pred_losses[:, self.num_adv:].sum(dim=1)).detach().cpu().numpy()
                elif self.intr_rew_options == '101': # intr rew from good agts only
                    intr_rew[:, i] += -pred_losses.sum(dim=1).detach().cpu().numpy()
                elif self.intr_rew_options == '110': # intr rew from adversaries only
                    intr_rew[:, i] += pred_losses.sum(dim=1).detach().cpu().numpy()
                else:  # no intr rew
                    print("Invalid intr rew options. Should use SACDMultiPolicy instead.")
