...
class SACDMultiWMVecPolicy(BasePolicy):
    ...
    def forward(self, batch: Batch,
        ...
        for i, actor in enumerate(self.actors):
            logit, h = actor(obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
    ...
    def calculate_intrinsic_reward(self, batch: Batch) -> Batch:
        ...
        if self.partial_obs and not self.only_self_pred:
            ...
            for i in range(self.num_adv, num_agents): # apply intr rew only on good agents
                obs_j_list = self.imagine_agent_j_obs(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim], i, j_list)
                ...
                true_next_obs_n = np.concatenate([batch.obs_next[:, i*self.obs_dim:(i+1)*self.obs_dim]] * len(j_list), axis=0) #(bs*j, obs_dim)
            ...
            for i in range(self.num_adv, num_agents):
                world_model = self.world_models[i]
                input_i = np.concatenate((batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim], np.expand_dims(batch.act[:,i], axis=-1)), axis=1)
                pred_next_obs_i = world_model(input_i)
                l2_losses[:, i] = np.linalg.norm(batch.obs_next[:, i*self.obs_dim:(i+1)*self.obs_dim] - pred_next_obs_i.detach().cpu().numpy(), axis=1)
    ...
    def _target_q(self, buffer: ReplayBuffer,
        ...
        with torch.no_grad():
            ...
            for i in range(len(self.actors)):
                ...
                critic2_old = self.critic2_olds[i]
                target_q = torch.min(
                        critic1_old(batch.obs_next[:, i*self.obs_dim:(i+1)*self.obs_dim]),
                        critic2_old(batch.obs_next[:, i*self.obs_dim:(i+1)*self.obs_dim]),
    ...
    def learn(self, batch: Batch, **kwargs) -> Dict[str, float]:
        ...
        for i in range(len(self.actors)):
            ...
            # critic 1
            current_q1 = critic1(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
            ...
            # critic 2
            current_q2 = critic2(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
            ...
            # actor
            current_q1a = critic1(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
            current_q2a = critic2(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
            logit, _ = actor(batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim])
            log_prob = torch.log(logit + self.__eps)
            ...
            world_model = self.world_models[i]
            inputs = np.concatenate((batch.obs[:, i*self.obs_dim:(i+1)*self.obs_dim], np.expand_dims(batch.act[:, i], axis=-1)), axis=1)
            pred_next_obs = world_model(to_torch(inputs, device=world_model.device, dtype=torch.float))
            true_next_obs = to_torch(batch.obs_next[:, i*self.obs_dim:(i+1)*self.obs_dim], device=world_model.device, dtype=torch.float)
