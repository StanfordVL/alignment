...
def world_model_policy_trainer(
        ...
        with tqdm.tqdm(total=step_per_epoch, desc=f'Epoch #{epoch}',
            ...
            while t.n < t.total:
                ...
                for i in range(min(
                    ...
                    global_step += 1
                    sample_data = train_collector.sample(batch_size)
                    losses = policy.learn(sample_data)
                    # wm_losses = world_model.learn(sample_data)
                    for k in result.keys():
                        data[k] = f'{result[k]:.2f}'
                    ...
                    for k in wm_losses.keys():
                        if stat.get(k) is None:
                            stat[k] = MovAvg()
                        stat[k].add(wm_losses[k])
                        data[k] = f'{stat[k].get():.6f}'
                        if writer and global_step % log_interval == 0:
                            writer.add_scalar(
                                k, stat[k].get(), global_step=global_step)
