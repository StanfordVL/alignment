...
from tianshou.policy import SACDMultiPolicy, SACDMultiCCPolicy, SACDMultiWMPolicy, SACDMultiCommPolicy, SACDMultiCommWMPolicy
...
def get_args():
    ...
    parser.add_argument('--num-render', type=int, default=10)
    parser.add_argument(
        '--device', type=str,
...
def visualize_multi_sacd(args=get_args()):
    ...
    if 'simple_tag' in params.task:
        ...
        log_keys.extend(['bench/collisions', 'bench/cr_dist', 'bench/adv_step_rew', 'bench/agt_step_rew'])
        ...
        benchmark_logger = SimpleAdversaryBenchmarkLogger(params.test_num, len([a for a in env.world.agents if a.adversary]))
        log_keys.extend(['bench/adv_occupied', 'bench/agt_occupied','bench/adv_dist', 'bench/agt_dist', 'bench/adv_step_rew', 'bench/agt_step_rew'])
    ...
    if args.final_model:
        video_file += '_final'
    video_file = video_file.replace('log', 'videos') + '.mp4' if args.auto_dir else args.video_file
    img_dir = args.logdir.replace('log', 'imgs') if args.auto_dir else args.img_dir
